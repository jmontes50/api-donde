...
async function create({ name, description, address, phone, image_url, opening_time, closing_time, lat, lng, district_id, category_id }) {
  const result = await db.execute({
    sql: `INSERT INTO restaurants (name, description, address, phone, image_url, opening_time, closing_time, lat, lng, district_id, category_id)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [name, description || null, address, phone || null, image_url || null, opening_time || null, closing_time || null, lat ?? null, lng ?? null, district_id, category_id],
  });
...
async function update(id, fields) {
  ...
  const allowed = ['name', 'description', 'address', 'phone', 'image_url', 'opening_time', 'closing_time', 'lat', 'lng', 'district_id', 'category_id'];
  const setClauses = [];
  const args = [];
